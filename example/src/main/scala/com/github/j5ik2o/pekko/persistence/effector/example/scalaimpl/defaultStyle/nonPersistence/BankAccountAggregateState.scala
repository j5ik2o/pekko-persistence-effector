package com.github.j5ik2o.pekko.persistence.effector.example.scalaimpl.defaultStyle.nonPersistence
...
enum BankAccountAggregateState {
  ...
  def applyEvent(event: BankAccountEvent): BankAccountAggregateState = (this, event) match {
    case (BankAccountAggregateState.NotCreated(aggregateId), BankAccountEvent.Created(id, limit, balance, _)) =>
      Created(id, BankAccount.create(id, limit, balance).bankAccount)
    case (BankAccountAggregateState.Created(id, bankAccount), BankAccountEvent.CashDeposited(_, amount, _)) =>
      bankAccount
        .add(amount)
        .fold(
    ...
    case (BankAccountAggregateState.Created(id, bankAccount), BankAccountEvent.CashWithdrew(_, amount, _)) =>
      bankAccount
        .subtract(amount)
        .fold(
          error => throw new IllegalStateException(s"Failed to apply event: $error"),
