...
public class BankAccountAggregate {
     ...
     * @return Actor behavior
     */
    public static Behavior<BankAccountCommand> create(BankAccountId aggregateId, PersistenceMode persistenceMode) {
        var config = PersistenceEffectorConfig
        ...
        return Behaviors.setup(ctx -> PersistenceEffector.fromConfig(config, (initialState, effector) -> {
    ...
    private static Behavior<BankAccountCommand> handleNotCreated(
            ...
            PersistenceEffector<BankAccountAggregateState, BankAccountEvent, BankAccountCommand> effector) {
        return Behaviors.receiveMessage(command -> {
            if (command instanceof BankAccountCommand.Create create) {
                var result = BankAccount.create(create.getAggregateId());
                return effector.persistEvent(result.getEvent(), newState -> {
                    create.replyTo().tell(CommandReply.CreateReply.succeeded(create.getAggregateId()));
                    var created = new BankAccountAggregateState.Created(state.getAggregateId(), result.getState());
                    return handleCreated(created, effector);
                });
            } else {
    ...
    private static Behavior<BankAccountCommand> handleCreated(
            ...
            PersistenceEffector<BankAccountAggregateState, BankAccountEvent, BankAccountCommand> effector) {
        return effector.persistSnapshot(state, newState -> Behaviors.receiveMessage(command -> {
                if (command instanceof BankAccountCommand.Stop stop) {
                    ...
                    getBalance.replyTo().tell(CommandReply.GetBalanceReply.succeeded(getBalance.getAggregateId(), state.bankAccount().getBalance()));
                    ...
                    if (result.isLeft()) {
                        depositCash.replyTo().tell(CommandReply.DepositCashReply.failed(depositCash.getAggregateId(), result.getLeft()));
                        return Behaviors.same();
                    } else {
                        var newBankAccount = result.getRight();
                        return effector.persistEvent(newBankAccount.getEvent(), newState2 -> {
                            depositCash.replyTo().tell(CommandReply.DepositCashReply.succeeded(depositCash.getAggregateId(), depositCash.amount()));
                            var created = new BankAccountAggregateState.Created(state.getAggregateId(), newBankAccount.getState());
                            return handleCreated(created, effector);
                        });
                    }
                    ...
                    if (result.isLeft()) {
                        withdrawCash.replyTo().tell(CommandReply.WithdrawCashReply.failed(withdrawCash.getAggregateId(), result.getLeft()));
                        return Behaviors.same();
                    } else {
                        var newBankAccount = result.getRight();
                        return effector.persistEvent(newBankAccount.getEvent(), newState2 -> {
                            withdrawCash.replyTo().tell(CommandReply.WithdrawCashReply.succeeded(withdrawCash.getAggregateId(), withdrawCash.amount()));
                            var created = new BankAccountAggregateState.Created(state.getAggregateId(), newBankAccount.getState());
                            return handleCreated(created, effector);
                        });
                    }
