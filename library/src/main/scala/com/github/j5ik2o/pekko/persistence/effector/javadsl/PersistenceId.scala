...
final class PersistenceId private (
   ...
   *
   * @return
   *   The entity type hint (null if not present)
   */
   ...
   *
   * @return
   *   Optional containing the entity type hint if present
   */
   ...
   *
   * @return
   *   The underlying Scala implementation
   */
...
object PersistenceId {
  ...
  def of(entityTypeHint: String, entityId: String, separator: String): PersistenceId =
    new PersistenceId(
      com.github.j5ik2o.pekko.persistence.effector.scaladsl.PersistenceId.of(entityTypeHint, entityId, separator))

  /**
   * Creates a PersistenceId from a Scala PersistenceId.
   *
   * @param scalaPersistenceId
   *   The Scala PersistenceId
   * @return
   *   A new Java PersistenceId instance
   */
  def fromScala(
    scalaPersistenceId: com.github.j5ik2o.pekko.persistence.effector.scaladsl.PersistenceId): PersistenceId =
