...
public class BankAccountAggregate {
        ...
        ctx -> {
          ...
          var config =
              PersistenceEffectorConfig
                  .<BankAccountAggregate.State, BankAccountEvent, BankAccountCommand>create(
                      actorName(aggregateId), new State.NotCreated(aggregateId), State::applyEvent)
  ...
  private static Behavior<BankAccountCommand> handleNotCreated(
    ...
    return Behaviors.receive(BankAccountCommand.class)
        .onMessage(
            BankAccountCommand.Create.class,
            cmd -> {
  ...
  private static Behavior<BankAccountCommand> handleCreated(
    ...
    ctx.getLog()
        .debug(
            "Handling Created state for aggregate: {}, balance: {}",
            state.getAggregateId(),
            state.bankAccount().getBalance());

    // Define basic behavior first
    var behavior =
        Behaviors.receive(BankAccountCommand.class)
            .onMessage(
                BankAccountCommand.Stop.class,
                cmd -> {
            ...
            .onMessage(
                BankAccountCommand.GetBalance.class,
                cmd -> {
            ...
            .onMessage(
                BankAccountCommand.DepositCash.class,
                cmd -> {
                  ctx.getLog()
                      .debug("Received DepositCash command: {}, amount: {}", cmd, cmd.amount());
                  ...
                  if (result.isLeft()) {
                    ...
                    return effector.persistEvent(
                        ...
                        event -> {
                          ctx.getLog()
                              .debug(
                                  "Event persisted callback, new balance: {}",
                                  r.getState().getBalance());
                          return handleCreated(
                              new State.Created(state.aggregateId(), r.getState()), effector, ctx);
                        });
                  }
                })
            .onMessage(
                BankAccountCommand.WithdrawCash.class,
                cmd -> {
                  ctx.getLog()
                      .debug("Received WithdrawCash command: {}, amount: {}", cmd, cmd.amount());
                  ...
                  if (result.isLeft()) {
                    ...
                    // Send response first
                    cmd.replyTo()
                        .tell(WithdrawCashReply.succeeded(cmd.aggregateId(), cmd.amount()));
            ...
            .onMessage(
                BankAccountCommand.StatePersisted.class,
                msg -> {
                  ctx.getLog().debug("Received StatePersisted message: {}", msg);
                  return Behaviors.same();
                })
            .onMessage(
                BankAccountCommand.EventPersisted.class,
                msg -> {
                  ...
                  if (!msg.events().isEmpty()) {
                    ...
                    if (event instanceof BankAccountEvent.CashDeposited deposited) {
                      ...
                      if (result.isRight()) {
                        return handleCreated(
                            new State.Created(state.aggregateId(), result.getRight().getState()),
                            effector,
                            ctx);
                      }
                    } else if (event instanceof BankAccountEvent.CashWithdrew withdrew) {
